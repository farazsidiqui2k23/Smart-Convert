...
const init = async () => {
  ...
  if (dom.toolGrid) {
    ...
    categories.forEach((category) => {
      ...
      title.className = 'text-xl font-bold mb-0 mt-8 first:mt-0 text-black';
      const categoryKey = categoryTranslationKeys[category.name];
      title.textContent = categoryKey ? t(categoryKey) : category.name;

      // ↓ yahan gradient line create karo
const underline = document.createElement('div');
underline.className =   'w-20 h-1 rounded-full mb-6 bg-gradient-to-r from-purple-500 to-indigo-500';
      ...
      category.tools.forEach((tool) => {
        ...
        icon.className = 'w-10 h-10 mb-3 text-indigo-100 bg-gradient-to-r from-purple-500 to-indigo-500 rounded-sm p-2';
        icon.setAttribute('data-lucide', tool.icon);

        const toolName = document.createElement('h3');
        toolName.className = 'font-semibold text-violet-700';
      ...
      categoryGroup.append(title, underline, toolsContainer);
