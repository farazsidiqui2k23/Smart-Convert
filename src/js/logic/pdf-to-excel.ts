...
const init = () => {
  ...
  if (convertBtn) {
    convertBtn.addEventListener('click', async () => {
      ...
      await convertToExcel(selectedFile);
    });
  }

  async function convertToExcel(file: File) {
    try {
      ...
      updateProgress(30, 'Converting to Excel...');

      const response = await fetch(`${API_URL}/convert/pdf-to-excel`, {
        method: 'POST',
        body: formData
      });
      ...
      updateProgress(80, 'Downloading Excel file...');
